...
- 003 tasks only modify `frontend/` and `specs/003-arbiter-workbench-ui/`.
- Do not modify `specs/001-regulation-structuring/` or `src/arbiter/schemas/regulation_structuring.py`.
...
**Purpose**: Establish the frontend-only Vite + React + TypeScript workspace for 003.
...
- [ ] T008 [P] Write contract tests for the `StructuringPipelineOutput` frontend validation mirror in `frontend/tests/contract/structuringOutput.contract.test.ts`
...
## Phase 4: User Story 2 - Preview Future Runtime Scenario Flow (Priority: P2)
...
## Phase 5: User Story 3 - Inspect Future Runtime Evidence And Trace (Priority: P3)
...
- [ ] T059 Verify with git diff, dependency inspection, and boundary tests that no forbidden 001 schema, 001 pipeline, 002 runtime, backend API, database, LLM, retrieval, vector search, rule execution, active `RulePack`, formal `RuleItem`, final `JudgmentResult`, or PDF/DOCX parsing changes were introduced in `frontend/`
...
- **User Story 2 (Phase 4)**: Depends on Foundational; can be implemented after or in parallel with US1, but MVP validation should finish US1 first.
- **User Story 3 (Phase 5)**: Depends on Foundational and benefits from US2 runtime draft display.
- **Polish (Phase 6)**: Depends on desired user stories being complete.

### User Story Dependencies

- **US1 - Curate Reviewed Regulation Data Assets**: No dependency on US2 or US3 after Foundational.
- **US2 - Preview Future Runtime Scenario Flow**: No dependency on US1 after Foundational; uses mocked runtime data only.
- **US3 - Inspect Future Runtime Evidence And Trace**: Depends on runtime placeholder contracts from Foundational and may reuse US2 draft display.
...
2. Add US2 to preview runtime scenario entry and mocked judgment draft display.
3. Add US3 to inspect mocked citations, evidence, sanitized traces, and review artifacts.
